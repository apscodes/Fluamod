...
{
	//*! Loads in the Lua Script File from FilePath
	int status = luaL_dofile(L, luaSFilePath);

	//*! If no Lua Script was found
	if (status)
	{
	...
	//*! Run the Start function in the script
	...
	//*! Error for a function that doesn't exist
	if (status)
	{
...
bool LuaObject::CallFunction(int argCount, int returnCount)
{
	...
	//*! Failed attempt at calling the function
	if (status)
	{
